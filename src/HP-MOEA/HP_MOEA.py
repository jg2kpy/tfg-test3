...
carpeta_resultados = 'modificacion2'
...
class Individuo():
    def __init__(self, new_matrix=None):
        global max_profit, max_novelty, max_relevance
    ...
    def get_fitness(self):
        self.fitness = [-self.get_profit() / max_profit, -self.get_novelty() / max_novelty, -self.get_relevance() / max_relevance]
        return self.fitness
    ...
    def _calculate_relevance(self):
        ...
        for user_id, lista_recomendacion in enumerate(self.matrix, start=1):
            for movie_id in lista_recomendacion:
                user_profile = users_profile.get(user_id)
                movie_content = movies_content.get(movie_id)

                resultado = sum(u for u, m in zip(user_profile, movie_content) if m == 1)

                relevance += (resultado / 190) # Top 10 del usuario y hay 19 categorias
        return relevance
...
def get_individual_max_novelty():
    ...
    for user in users:
        ...
        user_profile = users_profile.get(user.get_id())

        user_movie_affinity = {
            new_comers_id: sum(u for u, m in zip(user_profile, movies_content.get(new_comers_id)) if m == 1)
            for new_comers_id in posibles_ids
        }

        top_affinity_movies = sorted(user_movie_affinity.items(), key=lambda x: x[1], reverse=True)[:L_tamano_lista]
        matrix_max_novelty.append([movie_id for movie_id, _ in top_affinity_movies])

    individual_max_novelty = Individuo(matrix_max_novelty)
    max_novelty = individual_max_novelty.get_novelty()
    print(f"max_novelty: {max_novelty}")

    return individual_max_novelty

def get_individual_max_relevance():
    global max_relevance
    matrix_max_relevance = []
    ...
    for user in users:
        ...
        user_movie_affinity = {
            movie_id: sum(u for u, m in zip(user_profile, movies_content.get(movie_id)) if m == 1)
            for movie_id in posibles_ids
        }

        top_affinity_movies = sorted(user_movie_affinity.items(), key=lambda x: x[1], reverse=True)[:L_tamano_lista]
        matrix_max_relevance.append([movie_id for movie_id, _ in top_affinity_movies])

    individual_max_relevance = Individuo(matrix_max_relevance)
    max_relevance = individual_max_relevance.get_relevance()
    print(f"max_relevance: {max_relevance}")

    return individual_max_relevance
...
def calculate_population_metrics(population, log):
    ...
    # Normalización para NSGA-III, MOEA/D
    scaler = MinMaxScaler()
    normalized_fitness = scaler.fit_transform(fitness_of_population)
    ...
    for rank, front in enumerate(fronts):
        ...
        if algorithm_name == "NSGA-II":
            ...
            norm_front_fitness = normalized_fitness[front]
            crowding = [np.min(np.linalg.norm(p - ref_dirs, axis=1)) for p in norm_front_fitness]
...
# Probabilidad genetica para NSGA-III

# In[16]:


def lambda_NSGA3(A, B):
    rank_A = A.get_rank()
    rank_B = B.get_rank()
    ...
    # crowding_distance se interpreta como "distancia al ref_dir más cercano"
    dist_A = A.get_crowding_distance()
    dist_B = B.get_crowding_distance()
    if (dist_A + dist_B) != 0:
        # menor distancia es mejor → invertir proporción
        return dist_B / (dist_A + dist_B)
...
class HP_MOEA_Crossover(Crossover):
    ...
    def crossover(self, A_parent, B_parent, algorithm_name):
        ...
        lambda_genetic_probability = lambda_NSGA2(A_parent, B_parent) if algorithm_name == 'NSGA2' else lambda_NSGA3(A_parent, B_parent)
...
class HP_MOEA_Class():
    ...
    def generate_initial_population(self):
        ...
        individual_max_relevance = get_individual_max_relevance()
        ...
        initial_population.append(individual_max_relevance)
